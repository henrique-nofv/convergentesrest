...
import edu.up.bsi.conv.dao.CategoriaDAO;
...
public class CategoriaREST {
	...
	public Response gravarTarifa(@QueryParam(value = "veiculo") String tipoVeiculo, 
			@QueryParam(value = "eixos") int nroEixos,
			@QueryParam(value = "rodagem") String rodagem,
			@QueryParam(value = "tarifa") double MultTarifa,
			@QueryParam(value = "praca") int tipoPraca,
			@QueryParam(value = "valores") double ValoresPraticados,
			@QueryParam(value = "idCategoria") int idCategoria)throws Exception {

		if (tipoVeiculo == null || nroEixos == 0 || rodagem == null || MultTarifa == 0.0 || tipoPraca == 0 || idCategoria == 0) {
	...
	//	@Consumes(MediaType.WILDCARD)
	//	public Response buscarCategoriaPreco(@QueryParam(value = "praca") String praca, @QueryParam(value = "categoria") String categoriaId) throws Exception {
	...
	//	@Produces(MediaType.APPLICATION_XML)
	//	@Consumes(MediaType.WILDCARD)
	//	public Response buscarCategoria(@QueryParam(value = "praca") String praca, @QueryParam(value = "categoria") String categoriaId) throws Exception {
	//		System.out.println("praca:" + praca);
	//		System.out.println("categoriaId:" + categoriaId);
	//		List<Categoria> categorias;
	//		if(praca == "principal")
	...
	//			mensagem.setDescricao("O produto com código " + codigo + " foi removido com sucesso!");
	//			return Response.accepted().entity(mensagem).build();
	//		} catch (Exception e) {
	//			Mensagem mensagem = new Mensagem();
	//			mensagem.setStatus(Status.CONFLICT.toString());
	//			mensagem.setDescricao("O produto com código " + codigo + " não pôde ser removido!");
	...
	//			URI produtoAtualizado = baseUriBuilder.path(CategoriaREST.class).path(atualizado.toPath()).build();
	//			return Response.accepted(produtoAtualizado).entity(atualizado).build();
	//		} catch (Exception e) {
	//			Mensagem mensagem = new Mensagem();
	//			mensagem.setStatus(Status.CONFLICT.toString());
	//			mensagem.setDescricao("O produto " + produto.getNome() + " já está cadastrado!");
	//			return Response.status(Status.CONFLICT).entity(mensagem).build();
	//		}
	//	}
	...
	//	public Response buscarCategoriaPorId(@PathParam(value = "codigo") Long codigo) {
	//		Categoria pesquisado = dao.buscarCategoriaPorCodigo(codigo);
	//		if (pesquisado != null) {
	//			UriBuilder baseUriBuilder = uriInfo.getBaseUriBuilder();
	//			URI produtoPesquisado = baseUriBuilder.path(CategoriaREST.class).path(pesquisado.toPath()).build();
	...
	//	@Produces(MediaType.APPLICATION_JSON)
	//	@RolesAllowed(value = { "ADMIN", "USER", "GUEST" })
	//	public Response buscarCategoriaPorNome(@QueryParam(value = "nome") String nome) {
	...
	//		} catch (Exception e) {
	//			Mensagem mensagem = new Mensagem(Status.INTERNAL_SERVER_ERROR.toString(), "Problemas técnicos no sistema!");
